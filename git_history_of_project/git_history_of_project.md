...
This should be a very fast download. Now if you type `ls` within the repository directory, you should see a folder titled `planets`. Navigate into this directory using `cd`; this is the directory we will be exploring throughout the module.
...
If you are starting this module after following along with all of Dracula's code in the Creating and Tracking module, you can follow along here in your own planets repo.
...
To look at only the changes to the file about Venus, we can enter `git diff HEAD venus.txt`:
...
Even if the line is several sentences long and you only changed a single character, Git records that as removing the old line and adding in the new one.
...
The command `git diff HEAD venus.txt` will show you the differences between the current working version of `venus.txt` and the last committed version. 
If you happen to know the 6-digit commit number of your last commit, you could also use that. For example if the commit number was `123456`, then `git diff 123456 venus.txt` would give you the same result.
...
You can conclude from this output that you must have committed the file, and then added the last line to `venus.txt`.
